...
torchrun --standalone --nproc_per_node=$NPROC_PER_NODE -m scripts.base_train -- --depth=8 --target_param_data_ratio=20 --moe_layer_freq=2 --num_experts=8 --num_experts_per_tok=2 --device_batch_size=2 --moe_router_type=$MOE_ROUTER_TYPE --run=$WANDB_RUN --save_every 250 --model_tag=d8_fpe_direct
